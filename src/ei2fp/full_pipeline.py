...
if __name__ == "__main__":
    ...
    np.savetxt(BASE_DIR / "data/output/TST_fps.txt", tst_ds.fps.numpy()[:, FPS_MASK])
    np.savetxt(
        BASE_DIR / "data/output/VAL_maccs.txt", val_ds.maccs.numpy()[:, MACCS_MASK]
    )
    np.savetxt(BASE_DIR / "data/output/VAL_fps.txt", val_ds.fps.numpy()[:, FPS_MASK])
    trn_dl = DataLoader(
        trn_ds,
        batch_size,
        pin_memory=True,
